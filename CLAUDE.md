...
- Commit with hooks enabled (never use `--no-verify`). If pre-commit fails or is not installed, run manually: `uv run ruff check src tests lint_rules && uv run ruff format --check src tests lint_rules && uv run mypy src lint_rules && uv run ty check src lint_rules && uv run fixit lint src tests lint_rules && uv run lint-imports && uv run pytest`
